...
- **AI ワークスペースツール**: `cx`、`cc`、`mcp` による Codex / Claude Code
  profile、API router、health check、MCP sync、secret-safe switching。
...
| Windows | PowerShell profile hook と `cx`/`cc` helper | `Documents/PowerShell/Scripts/ai-env.ps1` |
...
- Windows PowerShell 用 `cx` / `cc` profile hook
...
Codex と Claude Code のローカル状態を切り替える軽量 shell function を提供します。
CLI 自体は起動しません。
...
ローカル Claude Docker container での Linux 検証:
